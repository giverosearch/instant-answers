...
DDH.timer.build = function(ops) {
    ...
    Timer = function (number, startingTime) {
        ...
        // dom setup
        this.$element = $(Handlebars.compile($("#timer").html())());
    ...
    function getTimeDurationFromQuery() {
        var res = 0;
        var query = IA.getQuery();
        var regexHours = /(?:(\d+)\s*(?:hours|hour|hrs|h|timer|time|t))/g;
        var regexMinutes = /(?:(\d+)\s*(?:minutes|minute|min|minutter|minut|n))/g;
        var regexSeconds = /(?:(\d+)\s*(?:seconds|second|sec|sekunder|sekund|s))/g;
        var hours = regexHours.exec(query);
        if (hours) {
            res += parseInt(hours[1]) * 3600;
        }
        var minutes = regexMinutes.exec(query);
        if (minutes) {
            res += parseInt(minutes[1]) * 60;
        }
        var seconds = regexSeconds.exec(query);
        if (seconds) {
            res += parseInt(seconds[1]);
        }
        return res;
    }
    ...
    function onShow() {
        ...
        var lastUpdate = new Date().getTime(),
            enteredTime = getTimeDurationFromQuery();
            // $dom = DDH.getDOM('timer'),
            // oldTitle = document.title;

        $addTimerBtn = $("#add_timer_btn");
